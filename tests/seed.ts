...
const companies: Company[] = [
    ...
    { name: "Microsoft", domain: "microsoft.com", category: "Technology", businessType: 'productivity' },
...
const emailStarts: string[] = ["noreply", "no-reply", "notifications", "updates", "welcome", "support", "billing", "security", "legal", "privacy", "newsletter", "updates", "welcome", "support", "billing", "security", "legal", "privacy", "newsletter"]
...
const getRandomPersonalEmail = (): PersonalEmail => personalEmails[Math.floor(Math.random() * personalEmails.length)];
...
const generateEmailBody = (category: string, params: Record<string, string>): string => {
  ...
  const categoryTemplates = emailTemplates[category as keyof typeof emailTemplates] || emailTemplates.Work;
...
const businessTypeEmailMap: Record<BusinessType, string[]> = {
  ...
  ecommerce: ['orders', 'shipping', 'support', 'noreply', 'returns', 'billing', 'promotions', 'sales'],
  finance: ['accounts', 'billing', 'statements', 'payments', 'security', 'noreply', 'transactions', 'support'],
  entertainment: ['updates', 'noreply', 'subscriptions', 'content', 'recommendations', 'watch', 'new'],
...
const generateRealisticContent = {
  ...
  words: (count: number, context: string): string => {
    const wordsByContext: Record<string, string[]> = {
      technology: ["cloud", "API", "integration", "scalability", "automation", "security", "deployment", "framework", "architecture", "infrastructure", "platform", "interface", "database", "algorithm", "analytics", "optimization", "encryption", "protocol", "backend", "frontend", "workflow", "repository", "compatibility", "authentication", "migration"],
      business: ["strategy", "growth", "ROI", "metrics", "conversion", "acquisition", "retention", "revenue", "investment", "scaling", "operations", "marketing", "branding", "partnership", "expansion", "compliance", "procurement", "diversification", "sustainability", "benchmark", "stakeholder", "valuation", "portfolio", "leadership", "innovation"],
      project: ["milestone", "deadline", "deliverable", "scope", "requirement", "timeline", "resource", "constraint", "objective", "outcome", "assessment", "iteration", "planning", "coordination", "implementation", "testing", "validation", "documentation", "maintenance", "collaboration", "verification", "approval", "specification", "monitoring", "evaluation"]
    };
    
    const fallbackWords = ["product", "service", "quality", "solution", "customer", "support", "experience", "development", "improvement", "update", "feature", "benefit", "efficiency", "performance", "reliability", "innovation", "value", "opportunity", "relationship", "communication"];
  ...
  productName: (): string => {
    const prefixes = ["Ultra", "Pro", "Elite", "Smart", "Power", "Flex", "Digital", "Hyper", "Eco", "Turbo", "Next", "Rapid", "Swift", "Core", "Prime"];
    const objects = ["Book", "Pad", "Hub", "Drive", "Connect", "Suite", "Cloud", "Box", "Link", "Pulse", "Guard", "Flow", "Boost", "View", "Sync"];
  ...
  department: (): string => {
    return faker.helpers.arrayElement([
      "Sales", "Marketing", "Engineering", "Product", "Customer Success", "Human Resources", 
      "Finance", "Operations", "Research & Development", "IT", "Legal", "Business Development", 
      "Design", "Quality Assurance", "Analytics", "Support", "Training", "Communications"
    ]);
  },
  ...
  businessVerb: (): string => {
    return faker.helpers.arrayElement([
      "optimize", "implement", "develop", "streamline", "launch", "analyze", "coordinate", 
      "facilitate", "integrate", "leverage", "manage", "enhance", "deploy", "accelerate", "transform", 
      "evaluate", "strategize", "innovate", "consolidate", "execute"
    ]);
  },
  ...
  businessNoun: (): string => {
    return faker.helpers.arrayElement([
      "strategy", "solution", "platform", "framework", "system", "process", "application", 
      "initiative", "interface", "architecture", "methodology", "infrastructure", "pipeline", 
      "dashboard", "protocol", "algorithm", "ecosystem", "repository", "model", "network"
    ]);
  },

  // Generate business phrases
  businessPhrase: (): string => {
    const adjectives = ["scalable", "robust", "innovative", "cutting-edge", "efficient", "strategic", "comprehensive", "integrated", "optimized", "streamlined", "agile", "responsive", "intuitive", "seamless", "dynamic"];
  ...
  familyEmailBody: (name: string, sender: string): string => {
    ...
    return faker.helpers.arrayElement(intros) + faker.helpers.arrayElement(contents) + faker.helpers.arrayElement(closings);
  },
...
const generateCompanyEmail = (category: string, userName = "User") => {
  ...
  const params: Record<string, string> = {
    ...
    service: faker.helpers.arrayElement(['Premium Plan', 'Basic Subscription', 'Pro Account', 'Enterprise Service', 'VIP Membership']),
...
const generatePersonalEmail = (category: string, userName = "User") => {
  ...
  // For personal emails, use primarily Social or Personal categories
  const effectiveCategory = person.relationship === 'colleague' 
    ? faker.helpers.arrayElement(['Work', 'Social', 'Personal']) 
    : faker.helpers.arrayElement(['Social', 'Personal']);
  
  const subject = person.relationship === 'colleague' 
...
const generateEmails = async (count: number, userId: string): Promise<{ emails: EmailWithMetadata[], threadsMap: Record<string, string[]> }> => {
