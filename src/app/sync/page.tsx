...
const SyncStats = ({ stats, syncInfo, currentStage }: { stats: SyncStats | null; syncInfo: SyncInfo; currentStage: SyncStage }) => {
    ...
    <Card fillWidth radius="m" border="neutral-alpha-weak" padding="24">
      <Column gap="16">
        <Row fillWidth gap="32">
          <Column flex={1}>
            <Text variant="label-default-s" onBackground="neutral-weak">Messages</Text>
            <Row horizontal="space-between" vertical="center">
              <Text>Processed:</Text>
              <Text variant="body-strong-m">{stats?.processedMessages ?? 0} / {stats?.totalMessages ?? 0}</Text>
            </Row>
          </Column>
          
          <Column flex={1}>
            <Text variant="label-default-s" onBackground="neutral-weak">New Messages</Text>
            <Row horizontal="space-between" vertical="center">
              <Text>Found:</Text>
        ...
        <Column>
          ...
          <Row horizontal="space-between" vertical="center">
            <Text>
              {getSyncStageDisplay(currentStage)}
            </Text>
          </Row>
          <Row horizontal="space-between" vertical="center">
            <Text>Estimated Time:</Text>
            <Text variant="body-strong-m">{stats?.estimatedTimeRemaining ?? "Calculating..."}</Text>
          </Row>
        </Column>
...
const SyncLogDisplay = ({ syncLog }: { syncLog: Array<{timestamp: string; message: string}> }) => {
  ...
  return (
    <>
      <Line background="neutral-alpha-weak" />
      <Column gap="8">
        <Text variant="label-default-s" onBackground="neutral-weak">Sync Log</Text>
        <Card background="neutral-weak" radius="s" padding="16" style={{ maxHeight: '150px', overflow: 'auto' }}>
          <Column gap="8">
            {syncLog.slice(-10).map((logEntry, index) => (
              <Text key={index} variant="body-default-xs" onBackground="neutral-strong">
                [{logEntry.timestamp.split('T')[1].split('.')[0]}] {logEntry.message}
              </Text>
            ))}
          </Column>
        </Card>
...
const getSyncStageDisplay = (stage: SyncStage): string => {
  switch (stage) {
    case SYNC_STAGES.MESSAGES: return "Message List Sync";
    case SYNC_STAGES.EMAILS: return "Email Content Sync"; 
    case SYNC_STAGES.SYNCING: return "Syncing Messages & Emails";
    case SYNC_STAGES.COMPLETE: return "Complete";
    case SYNC_STAGES.ERROR: return "Error";
    default: return "Idle";
  }
};
...
export default function SyncPage() {
  ...
  const startMessageStreamSync = async () => {
    try {
      ...
      setError(`Failed to start message sync: ${error instanceof Error ? error.message : String(error)}`);
      setSyncStage("error");
      addLogEntry(`Failed to start message sync: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  };
    ...
    try {
      ...
      while (true) {
        ...
        if (done) {
          ...
          addLogEntry(`${streamType === 'message' ? 'Message list' : 'Email content'} sync stream completed`);
      ...
      addLogEntry(`Error in ${streamType} stream: ${error instanceof Error ? error.message : String(error)}`);
      
      // Only update error state if we're still in a sync stage
      if (syncStage === "syncing") {
        setError(`Failed to read ${streamType} stream: ${error instanceof Error ? error.message : String(error)}`);
      }
  ...
  const handleStreamMessage = (data: any, streamType: 'message' | 'email') => {
    ...
    if (['error', 'batch-start', 'batch-complete', 'resume', 'complete', 'polling', 'aborted'].includes(data.type)) {
      addLogEntry(`${streamType === 'message' ? '[Messages]' : '[Emails]'} ${data.type}: ${data.message || JSON.stringify(data)}`);
    }
    ...
    switch (data.type) {
      case 'progress':
        ...
        if (streamType === 'message') {
          ...
          setStatus(`Syncing message list: ${data.processedMessages} of ${data.totalMessages}`);
          ...
          setStats(prev => ({
            ...
            estimatedTimeRemaining: calculateTimeRemaining(syncInfo.syncStartTime, data.progress / 100)
          }));
      ...
      case 'batch-start':
        ...
        setStatus(`${streamType === 'message' ? 'Processing message batch' : 'Processing email batch'} ${data.batchNumber}`);
        break;
      ...
      case 'resume':
        ...
        setStatus(`Resuming previous ${streamType} sync from batch ${data.batchNumber || 'last position'}`);
        break;
    ...
    if (isSyncing) {
      try {
        ...
        if (response.ok) {
          ...
          addLogEntry(`Error cancelling sync: ${data.error || data.message || 'Unknown error'}`);
        ...
        addLogEntry(`Error cancelling sync: ${error instanceof Error ? error.message : String(error)}`);
    ...
    if (isSyncing) {
      return (
        <Column gap="24">
          ...
          {stats && <SyncStats stats={stats} syncInfo={syncInfo} currentStage={syncStage} />}
      ...
      >
        ...
        >
          <Column fillWidth horizontal="center" gap="32" padding="32" position="relative">
            ...
            <Card fillWidth radius="xl" overflow="hidden" padding="32" border="neutral-alpha-weak">
              <Column gap="24">
                <Text align="center">
                ...
                <Column gap="16">
                  <Row gap="8" vertical="center">
                    ...
                    <Column>
                      <Text variant="body-strong-m">Retrieve message list from Gmail</Text>
                      <Text variant="body-default-s" onBackground="neutral-weak">
                        We'll get a list of all your message IDs (in batches of 500) and thread IDs.
                      </Text>
                    </Column>
                  </Row>
                </Column>
                ...
                {error && (
                  <Column background="danger-weak" radius="m" paddingY="16" paddingX="24" gap="4">
        ...
        <Column horizontal="center" paddingX="32" paddingY="64" fillWidth gap="16" position="relative">
          <Background
            mask={{
              cursor: true,
            }}
          ...
          <Card fillWidth padding="32" radius="xl" border="neutral-alpha-weak">
            <Column gap="24">
              <Heading as="h3" variant="display-default-s">About Message List Syncing</Heading>
              
              <Column gap="16">
                <Row gap="16" vertical="start">
                  <Icon name="security" size="l" />
                  <Text>
                    Your message metadata is encrypted before being stored in our database, ensuring 
                ...
                <Row gap="16" vertical="start">
                  ...
                  <Text>
                    After the initial sync, we'll periodically check for new messages to keep 
                    your inbox up to date.
                  </Text>
                </Row>
                ...
                <Row gap="16" vertical="start">
                  ...
                  <Text>
                    Local storage means faster access to your message list and reduced API calls 
                    to Gmail, improving performance.
                  </Text>
                </Row>
              </Column>
