...
export default function SyncPage() {
  ...
  const renderSyncContent = () => {
    if (isSyncing) {
      return (
        <Column gap="24">
          ...
          <Button variant="danger" onClick={cancelSync} disabled={!isSyncing} prefixIcon="close">
            Cancel Sync
          </Button>
        </Column>
      );
    ...
    if (syncStage === SYNC_STAGES.COMPLETE) {
      return (
        <Column gap="24">
          ...
          <Button variant="primary" disabled={false} onClick={() => router.push("/inbox")}>
            Go to Inbox
          </Button>
        </Column>
      );
    ...
    if (syncStage === SYNC_STAGES.ERROR) {
      return (
        <Column gap="24">
          ...
          <Button variant="primary" disabled={false} onClick={() => setSyncStage(SYNC_STAGES.IDLE)}>
            Try Again
          </Button>
        </Column>
      );
    ...
    return (
      <Column gap="24" horizontal="center">
        ...
        <Button variant="primary" disabled={isSyncing} onClick={startSync} loading={isSyncing}>
          Start Sync
        </Button>
      </Column>
    );
  ...
  const ProgressBar = ({ progress }: { progress: number }) => (
    <Row fillWidth background="neutral-alpha-weak" radius="full" height="8">
      <Row background="brand-medium" radius="full" height="8" style={{ width: `${progress}%` }} />
    </Row>
  );
        ...
        >
          <Column fillWidth horizontal="center" gap="32" padding="32" position="relative">
            ...
            <Card fillWidth radius="xl" overflow="hidden" padding="32" border="neutral-alpha-weak">
              <Column gap="24">
                <Text align="center">
                  This will sync your message list from Gmail to our database. Messages are
                  processed in batches of 500 at a time.
                </Text>
            ...
            <Column gap="24">
              ...
              <Column gap="16">
                ...
                <Row gap="16" vertical="start">
                  ...
                  <Text>
                    After the initial sync, we'll periodically check for new messages to keep your
                    inbox up to date.
                  </Text>
                </Row>
                ...
                <Row gap="16" vertical="start">
                  ...
                  <Text>
                    Local storage means faster access to your message list and reduced API calls to
                    Gmail, improving performance.
                  </Text>
                </Row>
              </Column>

              <Button variant="secondary" onClick={() => router.push("/inbox")} prefixIcon="inbox">
                Go to Inbox
              </Button>
            </Column>
          </Card>
