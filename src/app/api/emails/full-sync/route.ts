...
import { encryptText, encodeEncryptedData, extractContentFromParts } from "@/libs/utils";
...
const ongoingSyncs = new Map<string, { 
...
function getSyncStatus(userId: string) {
    ...
    const progress = totalMessages > 0 ? Math.min(100, Math.floor((processedMessages / totalMessages) * 100)) : 0;
    ...
    const estimatedSecondsRemaining = messagesPerSecond > 0 ? Math.floor(remainingMessages / messagesPerSecond) : 0;
...
export async function GET(request: NextRequest) {
    ...
    performFullSync(userId, accessToken, refreshToken, abortController.signal)
        .catch(error => {
...
async function performFullSync(
    ...
    try {
        ...
        const allMessageIds = await collectAllMessageIds(userId, accessToken, refreshToken, abortSignal);
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            const messageDetails = await fetchMessageBatch(userId, batch, accessToken, refreshToken);
            ...
            log(`Processed ${processedMessages}/${totalMessages} messages (${Math.floor((processedMessages / totalMessages) * 100)}%)`);
        ...
        log(`Full email sync completed for user ${userId}. Processed ${processedMessages}/${totalMessages} messages`);
    } catch (error) {
...
async function collectAllMessageIds(
    ...
    do {
        ...
        log(`Collected batch of ${messageIds.length} message IDs, total: ${allMessageIds.length}, nextPageToken: ${pageToken ? 'yes' : 'no'}`);
