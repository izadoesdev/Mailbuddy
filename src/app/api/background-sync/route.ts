 ...
 */
async function getOrCreateSyncState(userId: string): Promise<{ historyId: string | null, id: string }> {
...
async function processUserChanges(
  ...
  try {
    ...
    for (const record of historyResponse.data.history || []) {
      ...
      for (const modified of [...(record.labelsAdded || []), ...(record.labelsRemoved || [])]) {
    ...
    for (let i = 0; i < messageArray.length; i += SYNC_BATCH_SIZE) {
      ...
      log(`Processed batch ${i/SYNC_BATCH_SIZE + 1}/${Math.ceil(messageArray.length/SYNC_BATCH_SIZE)}, ${successful}/${batch.length} successful`);
    }
    
    log(`Successfully processed ${processedCount}/${messageIds.size} messages for user ${userId}`);
    ...
    if (error?.response?.status === 404 && error?.response?.data?.error?.message?.includes("Start history ID")) {
