...
const createRedisClient = (
  ...
  console.debug(`Creating Redis client with URL: ${url.replace(/(redis:\/\/[^:]+:)([^@]+)(@.+)/, '$1****$3')}`);
      ...
      try {
        ...
        if (cached) {
          ...
          if (staleWhileRevalidate) {
            ...
            if (ttl < staleTime) {
              // Return stale data and revalidate in background
              fn(...args).then(async (freshData: any) => {
                if (freshData !== undefined && freshData !== null) {
                  await getRedisCache().setex(key, expireInSec, serialize(freshData));
                }
              }).catch((error: any) => {
                console.error(`Background revalidation failed for function ${fn.name}:`, error);
              });
            }
          }
        ...
        retries++;
        if (retries === maxRetries) {
          console.error(`Cache error for function ${fn.name} after ${maxRetries} retries:`, error);
          return fn(...args);
        }
        await new Promise(resolve => setTimeout(resolve, 100 * retries)); // Exponential backoff
      }
