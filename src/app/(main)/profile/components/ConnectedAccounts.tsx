...
import { Button, Card, Column, Heading, Icon, Row, Spinner, Tag, Text } from "@/once-ui/components";
...
export default function ConnectedAccounts({ user }: ConnectedAccountsProps) {
    ...
    const loadAccounts = useCallback(async () => {
        try {
            ...
            const formattedAccounts = Array.isArray(accountList)
                ? accountList.map((acct) => ({
                      id: acct.id || acct.accountId || "",
                      providerId: acct.provider || "",
                      providerName: acct.provider || "",
                      createdAt: acct.createdAt
                          ? new Date(acct.createdAt).toISOString()
                          : new Date().toISOString(),
                      updatedAt: acct.updatedAt
                          ? new Date(acct.updatedAt).toISOString()
                          : new Date().toISOString(),
    ...
    const availableProviders = [{ id: "google", name: "Google", icon: "google" }] as const;
    ...
    return (
        <>
            <Column paddingX="24" paddingTop="24" fillWidth>
                <Text>
                    Connect additional accounts to your profile for simplified sign-in. You can use
                    any connected account to access your account.
                </Text>
                ...
                >
                    <Column gap="16">
                        <Row gap="16" vertical="center">
                            ...
                            <Text variant="body-strong-m">
                                Use Mailbuddy as your email provider
                            </Text>
                        </Row>
                        <Text variant="body-default-s">
                            Soon you'll be able to use Mailbuddy as your primary email provider with
                            your own custom domain. Enjoy seamless integration with all our features
                            while maintaining full control over your email ecosystem.
                        </Text>
                    </Column>
                </Row>
            </Column>
