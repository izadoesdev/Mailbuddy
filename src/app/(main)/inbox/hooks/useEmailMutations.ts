    ...
    const trashEmail = useMutation<string, Error, string, MutationContext>({
        ...
        onMutate: async (emailId) => {
            ...
            queryClient.setQueriesData({ queryKey: ['inbox'] }, (oldData: any) => {
                ...
                const updatedThreads = oldData.threads.map((thread: any) => {
                    if (!thread?.emails) return thread;
                    
                    // Check if this email is in the thread
                    if (thread.emails.some((email: any) => email?.id === emailId)) {
                        const remainingEmails = thread.emails.filter((email: any) => email.id !== emailId);
            ...
            queryClient.setQueryData(["toasts"], [
                {
                    ...
                    variant: "danger",
                    message: err instanceof Error ? err.message : "Failed to move email to trash"
                }
            ]);
        },
        onSuccess: () => {
