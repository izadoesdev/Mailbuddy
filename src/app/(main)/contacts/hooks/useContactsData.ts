...
import type { Contact, ContactsResponse, ContactsQueryParams } from "../types";
import { useRef } from "react";

/**
 * Fetches contacts from the API
 */
async function fetchContacts(params: ContactsQueryParams, signal?: AbortSignal): Promise<ContactsResponse> {
  // Build query string
  ...
  try {
    ...
    return {
      ...
      contacts: data.contacts?.map((contact: Contact) => ({
        ...contact,
        latestEmailDate: contact.latestEmailDate ? new Date(contact.latestEmailDate) : null
      })) || []
    };
  } catch (error) {
...
export function useContactsData(params: ContactsQueryParams = {}) {
  ...
  const {
    ...
    failureCount,
    status
  } = useQuery<ContactsResponse, Error>({
    queryKey,
