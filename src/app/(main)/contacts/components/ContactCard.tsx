...
import { Card, Text, Avatar, Row, Column, Tag, Line, Icon } from '@/once-ui/components';
...
export function ContactCard({ contact, onClick, selected }: ContactCardProps) {
  ...
  const dominantCategory = Object.entries(contact.categories)
    .sort(([, countA], [, countB]) => countB - countA)
    .map(([category]) => category)[0] || "Uncategorized";
  
  // Get dominant priority
  const dominantPriority = Object.entries(contact.priorities)
    .sort(([, countA], [, countB]) => countB - countA)
      ...
      <Column gap="16" fillWidth>
        <Row gap="16" vertical="center" fillWidth>
          ...
          <Column gap="4" fill>
            ...
            <Text variant="body-default-s" onBackground="neutral-weak">{contact.email}</Text>
          </Column>
        </Row>
        ...
        <Row wrap gap="16" vertical="center">
          <Column gap="4">
            <Text variant="label-default-xs" onBackground="neutral-weak">EMAILS</Text>
            <Text variant="body-strong-m">{contact.emailCount}</Text>
          </Column>
          
          <Column gap="4">
            <Text variant="label-default-xs" onBackground="neutral-weak">THREADS</Text>
            <Text variant="body-strong-m">{contact.threadCount}</Text>
          </Column>
          
          <Column gap="4">
            <Text variant="label-default-xs" onBackground="neutral-weak">LAST CONTACT</Text>
          ...
          {contact.unreadCount > 0 && (
            <Column gap="4">
              <Text variant="label-default-xs" onBackground="neutral-weak">UNREAD</Text>
              <Tag
                variant="danger"
        ...
        <Row gap="8" wrap>
          <Tag 
            variant="neutral"
            label={`Category: ${dominantCategory}`}
          />
          <Tag 
            variant={PRIORITY_COLORS[dominantPriority as keyof typeof PRIORITY_COLORS] || "neutral"}
